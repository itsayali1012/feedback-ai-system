...
const pool = new Pool({
  ...
  max: 5,
  connectionTimeoutMillis: 10000, // 10 second timeout for Neon
});
...
function mockQuery(text, params = []) {
  ...
  if (normalizedText.includes('SELECT id, rating, review, ai_summary, recommended_action, user_response, created_at, status FROM feedback_submissions')) {
    ...
    return {
      rows: filteredSubmissions.map(sub => ({
        ...sub,
        created_at: sub.created_at instanceof Date ? sub.created_at.toISOString() : sub.created_at
      }))
    };
  }
  ...
  if (text.includes('INSERT INTO feedback_submissions')) {
    const newSubmission = {
      ...
      rating: params[0],
      review: params[1] || '',
      ai_summary: params[2] || 'Mock AI summary - please configure Gemini API key for real summaries',
      recommended_action: params[3] || 'Mock recommended action - please configure Gemini API key for real recommendations',
      user_response: params[4] || 'Mock user response - please configure Gemini API key for real responses',
      created_at: new Date(),
      status: 'completed'
    };
