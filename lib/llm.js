...
console.log(
  '[LLM INIT]',
  'key exists:', !!process.env.GEMINI_API_KEY,
...
if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'your-gemini-api-key') {
  try {
    ...
    // Try different models with potentially better free tier limits
    const modelsToTry = ['gemini-2.0-flash-exp', 'gemini-2.0-flash-lite', 'gemini-flash-lite-latest', 'gemini-2.0-flash'];

    for (const modelName of modelsToTry) {
      try {
        model = genAI.getGenerativeModel({ model: modelName });
        console.log(`✅ Gemini initialized with ${modelName}`);
        break;
      } catch (error) {
