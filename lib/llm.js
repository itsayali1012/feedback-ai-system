...
export async function generateSummary(review) {
  ...
  // If OpenAI is not configured, return realistic mock summary
  if (!openai) {
    console.log('OpenAI not configured, using realistic mock summary');
...
export async function generateRecommendedAction(rating, review, summary) {
  ...
  // If OpenAI is not configured, return realistic mock action
  if (!openai) {
    console.log('OpenAI not configured, using realistic mock recommended action');
...
export async function generateUserResponse(rating, review) {
  ...
  // If OpenAI is not configured, return realistic mock response
  if (!openai) {
    console.log('OpenAI not configured, using realistic mock user response');
    ...
    const responses = {
      ...
      2: [
        ...
        "Thank you for your honest feedback. We're truly sorry that your experience didn't meet your expectations. We'd welcome the chance to discuss this further and work on improving our service."
      ],
      1: [
